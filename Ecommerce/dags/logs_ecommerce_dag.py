...
def generer_logs_journaliers(**context):
    ...
    if not os.path.exists(fichier_sortie):
        raise FileNotFoundError(f"{fichier_sortie} introuvable")
    size = os.path.getsize(fichier_sortie)
    logging.info(f"Fichier généré: {fichier_sortie} ({size} bytes)")
...
def brancher_selon_taux_erreur(**context):
    ...
    if taux_pct > SEUIL_ERREUR_PCT:
        logging.warning(f"Taux d'erreur {taux_pct:.2f}% > seuil {SEUIL_ERREUR_PCT}% → ALERTE")
        return "alerter_equipe_ops"
    else:
        logging.info(f"Taux d'erreur {taux_pct:.2f}% <= seuil {SEUIL_ERREUR_PCT}% → OK")
...
with DAG(
    ...
    t_upload = BashOperator(
        ...
        docker exec namenode hdfs dfs -put -f \
            /tmp/access_${EXECUTION_DATE}.log \
    ...
    bash_command="""
    EXECUTION_DATE="{{ ds }}"
    CHEMIN="/data/ecommerce/logs/raw/access_${EXECUTION_DATE}.log"
    echo "[INFO] Vérification présence fichier HDFS : ${CHEMIN}"
    docker exec namenode hdfs dfs -test -e ${CHEMIN} && echo "[OK] Fichier présent" || exit 1
    """,
)
